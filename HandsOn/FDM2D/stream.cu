...
__device__ double psi_old[181][271],pE[181][271];
...
int main(void)
{
  ...
  err=cudaMemcpyToSymbol(beta, &h_beta,sizeof(double));
...
template<int col> __global__ void Initial(int N, int M, double *psi_new)
{
  ...
  if(idx_x<M && idx_y<N) pP[idx_y][idx_x]=0.0;
...
template<int row, int col> __global__ void Jacobi_iter(const int N, const int M, double *psi_new)
{
  ...
  {
    if(idx_x<M && idx_y<N) psi_old[idx_y][idx_x]=pP[idx_y][idx_x];
	__threadfence();
    ...
    if(idx_x<M && idx_y<N){
        pE[idx_y][idx_x]=
			(pP[idx_y][idx_x]-psi_old[idx_y][idx_x])*(pP[idx_y][idx_x]-psi_old[idx_y][idx_x]);
	}
	__threadfence();

	error=thrust::reduce(thrust::seq, (double*)pE, (double*)pE+M*N,0.0,thrust::plus<double>());
    error = sqrt(error/(M*N));
    if(idx_y==0 && idx_x==0 && iter%1000==0) printf("Iteration = %ld, Error= %lg\n",iter,error);

    if(error<=EPSILON) break;
  }
  if(idx_y==0 && idx_x==0) printf("Iteration = %ld, Error= %lg\n",iter,error);    
}
