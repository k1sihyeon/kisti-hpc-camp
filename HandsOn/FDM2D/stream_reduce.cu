...
//const int M=90*4+1, N=60*4+1; // # of grid points starting from 0 in x-, y-direction
...
__device__ double psi_old[N][M],pE[N][M];
...
template<int row, int col> __global__ void Jacobi_iter(const int N, const int M, double *psi_new)
{
  ...
  for(iter=1;iter<MAX_ITER;++iter)
  {
	...
	error=thrust::reduce(thrust::device, (double*)pE, (double*)pE+M*N,0.0,thrust::plus<double>());
